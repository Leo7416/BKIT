...
HELP = '''
/start - Начать
/preview - Миниролик про команду
/photo - Фото текущего состава команды
'''
...
def start(message):
    ...
    phrase1 = types.KeyboardButton('Круто, я тоже болею за GSW')
...
def preview(message):
    video = open('D:\PyCharm\Lab6\Preview.mp4', 'rb')
...
def answer(message):
    if (message.text == 'Я не являюсь фанатом GSW'):
    ...
    elif (message.text == 'Круто, я тоже болею за GSW'):
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=1)
        phrase0 = types.KeyboardButton('Вернутся назад')
        ...
        bot.send_message(message.chat.id, 'Вы вернулись', reply_markup=markup3)
